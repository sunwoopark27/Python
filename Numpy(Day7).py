...
# 1. 1부터 10까지의 정수를 갖는 1차원 배열을 생성하고, 각 원소에 대해 제곱 값을 출력해보자.
one = np.array(1,11)
print(one**2)

# 2. 3x3 크기의 단위 행렬(identity matrix)을 생성하고, 이 행렬과 2x3 크기의 랜덤 행렬을 곱해 보자.
two = np.eye(3)
two_rd = np.random.rand(2,3)
two@two_rd.T

# 3. 아래의 배열에 대한 최소값과 최대값은?
[[12, 21, 34, 26, 28],
 [35,  8, 10,  6, 30],
 [47, 35, 12, 44, 11],
 [36, 39, 10, 48,  1],
...
# 4. 아래의 배열의 다음을 출력해보자 (2번째 행, 4번째 열), (첫번째 행), (마지막 열)
four = np.array([[17, 43, 10,  9, 22],
[44, 10,  9, 31, 27],
[26, 35, 12,  8, 41],
[43, 36, 25, 29, 21]])
...
# 5. 아래의 배열에서 대각선의 값을 모두 선택
five = np.array([[28,  2, 11],
 [27, 27, 21],
 [11,  5, 24]])

# 5-1
five_a = five[[0,1,2],[0,1,2]]
# 5-2
five_bool = np.eye(3)
five[five_bool == 1]

# 6. 아래의 배열에 대하여 값이 3의 배수인 경우만 출력해보자.
six = np.array([[11, 91, 96, 77],
 [70, 96, 83, 13],
 [63, 80, 32, 27],
 [17, 17, 63, 16]])

six[six%3==0] # array([96, 96, 63, 27, 63])

# 7. 아래의 배열에 대하여 다음을 값을 출력해보자.
#    전체합, 행별 평균, 축별 표준편차, 행별 누적합, 축별 누적곱
...
# 첫행 평균
seven_one = seven[0,:]
seven_one.sum()/len(seven_one)
# 아예 평균을 구해주는 함수
seven.mean(axis=1) # array([38. , 27.6, 60.6])
